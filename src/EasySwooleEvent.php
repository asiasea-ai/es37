...
class EasySwooleEvent
{
    ...
    public static function mainServerCreate(EventRegister $register): void
    {
        ...
        try {
            /** 策略加载 */
            Policy::getInstance()->initialize(EsConst::ES_POLICY_CONF_IS_AUTH);

            // 项目把 POLICY_CONF_* 常量从 AppConst 挪到 EsConst,vendor 这里反射目标改为 EsConst
            // (登记 Doc/vendor/Es3.md;原版反射 AppConst::POLICY_CONF_IS_SIGN)
            $policyConfIsSign = (new \ReflectionClass(EsConst::class))->getConstant('POLICY_CONF_IS_SIGN');
            if ($policyConfIsSign) {
                Policy::getInstance()->initialize(EsConst::ES_POLICY_CONF_IS_SIGN);
            }
